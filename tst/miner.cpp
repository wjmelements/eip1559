...
void test_miner_index() {
    initMiners(3);
    block_t *block0 = mineBlock(&miners[1], 1, 1);
    assert(block0->miner == 1);
    assert(block0->parent == NULL);
    assert(block0->difficulty == 1);
    block_t *block1 = mineBlock(&miners[2], 17, 2);
...
void test_mineBlock() {
    ...
    block_t *genesis = mineBlock(&miners[0], 300, 1000);
    assert(genesis != NULL);
    assert(genesis->txCount > 1);
    validateBlock(genesis);
    block_t *separate = mineBlock(&miners[1], 301, 1000);
    assert(separate->parent == NULL);
    assert(separate != NULL);
    assert(separate->txCount < 2);
    validateBlock(separate);
    return;
}
